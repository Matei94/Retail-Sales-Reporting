...
void task1_5(LinkedList<Produs> listaProduse, LinkedList<Bon> listaBonuri) {
    ...
    // Alocare vector de pointeri, pentru o matrice triunghiulara, nu are rost sa luam matrice intreaga, ca e simetrica fata de DP
	int **matrice = new int*[ numarProduse - 1];                       

    /* Vectorul vizitat memoreaza daca am vizitat un nod din listaBonuri */
	int *vizitat = new int[numarLiniiBonuri];
    ...
    */
    ...
    /* Initializare matrice triunghiulara */
	for ( i = 0; i < numarProduse - 1; i++) {
        matrice[i] = new int[i];  
    }
    ...
    while ( auxiliar != NULL) {
    	...
    	for (i = 0; i < indiceProduseBon - 1; i++) {
    			id1 = produseBon[i];

    	 	for (j = i + 1;i < indiceProduseBon; j++) {
                ...
                /* In matrice cautam in acelasi timp maximul de aparitii pentru o pereche.
    	 		...
    	 		if (id1 > id2) {
    				...
    				if( matrice[id1 - 2][id2 - 1] > maxim) {
    					maxim = matrice[id1 - 2][id2 - 1];
    					id1max = id1;
    					id2max = id2;
    				}
	    		}
                    ...
                    /* Aici se procedeaza la fel*/
	   				...
	   				if( matrice[id2 - 2][id1 - 1] > maxim) {
    					maxim = matrice[id2 - 2][id1 - 1];
    					id1max = id1;
    					id2max = id2;
    				}
	    		}
